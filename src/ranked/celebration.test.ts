...
describe('rankedCelebration', () => {
  it('celebrates a first score in the top ranks as a best, a fresh placement, and its board', () => {
    expect(
      rankedCelebration(
        result({ rank: 5, previousRank: null, isPersonalBest: true }),
        'faster:hard',
      ),
  ...
  it('celebrates a personal best outside the top ranks without a rank but with its board', () => {
    expect(
      rankedCelebration(
        result({ rank: 42, previousRank: 88, isPersonalBest: true }),
        'faster:hard',
      ),
  ...
  it('celebrates a climb into the top ranks from outside', () => {
    expect(
      rankedCelebration(
        result({ rank: 8, previousRank: 15, isPersonalBest: true }),
        'howfast:standard',
      ),
    ).toEqual({
      streak: 10,
  ...
  it('keeps the placement (not climbed) on a best that holds the rank (beat your own best while #1)', () => {
    expect(
      rankedCelebration(
        result({ rank: 1, previousRank: 1, isPersonalBest: true }),
        'faster:standard',
      ),
    ).toEqual({
      streak: 10,
  ...
  it('returns null when the run set neither a best nor a better placement', () => {
    expect(
      rankedCelebration(
        result({ rank: 12, previousRank: 12, isPersonalBest: false }),
        'faster:hard',
      ),
    ).toBeNull();
  });
